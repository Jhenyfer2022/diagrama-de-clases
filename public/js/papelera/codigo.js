...
class Figura {
  ...
  // Método para calcular y actualizar la altura de la caja
  actualizarAlto() {
    textSize(this.tamañoDeTextoNombre);
    let alturaNombre = textAscent() + textDescent(); // Altura del texto del nombre

    textSize(this.tamañoDeTextoMetodo);
    let alturaMetodo = textAscent() + textDescent(); // Altura del texto del estereotipo

    textSize(this.tamañoDeTextoDescripcion);
    let alturaDescripcion = textAscent() + textDescent(); // Altura del texto de la descripción

    // Calcula el alto total necesario
    let lineasDescripcion = Math.ceil(textWidth(this.descripcion) / (this.ancho - 20)); // Estima el número de líneas de la descripción
    // let lineasDescripcion = dibujarTextoEnLineas();
    let alturaDescripcionTotal = alturaDescripcion * lineasDescripcion;

    this.alto = alturaNombre + alturaMetodo + alturaDescripcionTotal + 60 + (lineasDescripcion - 1) * 10; // Añade un margen adicional para separación
    this.centroY = this.y + this.alto / 2; // Recalcula el centroY después de ajustar la altura
  }
  ...
  set setDescripcion(descripcion) {
    this.descripcion = descripcion;
    //this.actualizarAlto(); // Recalcula la altura cuando se cambia la descripción
  }
  get getDescripcion() {
  ...
  set setAncho(ancho) {
    this.ancho = ancho;
    //  this.actualizarAlto(); // Recalcula la altura cuando se cambia el ancho
    this.centroX = this.x + this.ancho / 2;
  }
  get getAncho() {
    return this.ancho;
  ...
  dibujarTexto() {
    ...
    // Dibuja el nombre de la clase
    textStyle(BOLD);
    textSize(this.tamañoDeTextoNombre);
    const nombreY = this.getY + this.tamañoDeTextoNombre / 2;
    text(this.getNombre, this.getCentroX, nombreY);

    // Dibuja una línea debajo del nombre
    let posicionYLinea = this.getY + this.tamañoDeTextoNombre + 10; // Posición para la línea
    this.dibujarLineaSeparadora(posicionYLinea);

    // Dibuja los atributos (descripción)
    textStyle(NORMAL);
    textSize(this.tamañoDeTextoDescripcion);
    let descripcionInicioY = posicionYLinea + 30; // Ajusta la posición de inicio de la descripción

    // Almacena la posición actual para la descripción
    let posicionActualY = descripcionInicioY;

    // Ajustar la posición de cada línea de la descripción
    let lineasDescripcion = this.dibujarTextoEnLineas(this.getDescripcion, this.getX + 50, posicionActualY, this.getAncho - 20);

    // Sumar la altura de las líneas de descripción
    let alturaDescripcion = lineasDescripcion * (textAscent() + textDescent()); // Altura total de la descripción
    posicionActualY += alturaDescripcion + lineasDescripcion; // Mover hacia abajo para el margen adicional

    // Dibuja la línea separadora debajo de la descripción
    this.dibujarLineaSeparadora(posicionActualY);

    // Dibuja los métodos debajo de la descripción
    textSize(this.tamañoDeTextoMetodo);
    let metodoInicioY = posicionActualY + 20; // Ajustar la posición para los métodos
    let lineasMetodo = this.dibujarTextoEnLineas(this.getMetodo, this.getX + 50, metodoInicioY, this.getAncho - 20);

    pop();
  }

  dibujarTextoEnLineas(texto, x, y, anchoMaximo) {
    let palabras = texto.split(' ');  // Divide el texto en palabras
    let linea = '';  // Inicializa una línea vacía
    let lineas = 0;  // Contador de líneas

    for (let i = 0; i < palabras.length; i++) {
      let pruebaLinea = linea + palabras[i] + ' ';  // Prueba agregar la palabra actual a la línea

      if (textWidth(pruebaLinea) > anchoMaximo) {
        // Si la línea es más ancha que el ancho máximo, dibuja la línea actual
        text(linea, x, y);
        linea = palabras[i] + ' ';  // Comienza una nueva línea con la palabra actual
        y += textAscent() ;  // Avanza la posición en Y
        lineas++;  // Incrementa el contador de líneas
      } else {
        linea = pruebaLinea;  // Si cabe en la línea, actualiza la línea
      }
    }

    // Dibuja la última línea restante si hay contenido
    if (linea.length > 0) {
      text(linea, x, y);
      lineas++;  // Cuenta la última línea dibujada
    }

    return lineas;  // Devuelve el número total de líneas
  }
  ...
  dibujarLineaSeparadora(y) {
    stroke(0); // Color de la línea (blanco)
    strokeWeight(1); // Grosor de la línea
    line(this.getX, y, this.getX + this.getAncho, y); // Dibuja la línea horizontal
  }
...
class Persona extends Figura {
  ...
  draw() {
    push();
    fill(255, 255, 255)
    rect(this.x, this.y, this.ancho, this.alto, 0)
    super.dibujarTexto()
