    ...
    if (!email || !pass) {
      ...
      conection.query(
        ...
        async (error, results) => {
          ...
          conection.query('UPDATE users SET is_connected = true WHERE Id = $1', [results.rows[0].id], (error) => {
            if (error) {
              console.error('Error al actualizar estado de conexión:', error);
            }
            // Una vez conectado, redirige al lienzo
            // req.user = { id: userId, email }; // Suponiendo que almacenas el usuario en req
            //   res.redirect(`/lienzo/${results.rows[0].id}`); // Redirige al lienzo
          });

          console.log(results.rows.length);
          idLogout = results.rows[0].id;
...
exports.logout = (req, res) => {
  ...
  console.log("Este es el id: " + idLogout)
  // Actualizar el estado de conexión
  conection.query('UPDATE users SET is_connected = false WHERE Id = $1', [idLogout], (error) => {
    if (error) {
      console.error('Error al actualizar estado de conexión:', error);
    }
